...
size_t qemu_write(const void *buffer, size_t len)
{
    ...
    xSemaphoreGive(_qemu_mutex);
    KERN_LOG("QEMU", APP_LOG_LEVEL_ERROR, "tTX %d", len);
...
void qemu_send_data(uint16_t endpoint, uint8_t *data, uint16_t len)
{
    ...
    KERN_LOG("QEMU", APP_LOG_LEVEL_ERROR, "TX");
    xSemaphoreGive(_qemu_mutex);
}
...
static void _qemu_thread(void *pvParameters)
{
    for (;;)
    {
        ...
        KERN_LOG("QEMU", APP_LOG_LEVEL_ERROR, "RX");
...
static void _qemu_handle_packet(void)
{
    ...
    {
        KERN_LOG("QEMU", APP_LOG_LEVEL_INFO, "More Data Required %d %d", header.len, protocol_get_rx_buf_size());
        return;
    }
